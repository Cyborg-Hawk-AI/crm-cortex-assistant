...
import { BookOpen, Plus, MoreVertical, Book, ChevronLeft } from 'lucide-react';
...
export function MindboardSidebar({ 
  ...
  // Get the sidebar state from context if available
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-3 border-b border-[#3A4D62]">
        ...
        <Button 
          ...
          size="sm" 
          className={cn(
            "h-8 w-8 p-0 text-neon-blue hover:text-neon-aqua hover:bg-[#3A4D62]/50",
          ...
          {mindboards.length === 0 ? (
            ...
            mindboards.map((mindboard) => (
              <motion.div
                ...
                className={`flex items-center ${isCollapsed ? 'p-1 justify-center' : 'p-2'} rounded-md mb-1 cursor-pointer text-sm ${
              ...
              >
                <Book className={`h-4 w-4 ${!isCollapsed && 'mr-2'} ${
