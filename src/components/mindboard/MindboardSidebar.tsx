...
import { BookOpen, Plus, MoreVertical, Book, ChevronRight } from 'lucide-react';
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-3 border-b border-[#3A4D62]">
        ...
        <Button 
          ...
          variant="ghost" 
          size="sm" 
          className={`h-8 w-8 p-0 text-neon-blue hover:text-neon-aqua hover:bg-[#3A4D62]/50 ${isCollapsed ? 'mx-auto' : ''}`}
      ...
      <ScrollArea className="flex-1">
        <div className="p-2">
          {mindboards.length === 0 ? (
            <div className={`flex flex-col items-center justify-center ${isCollapsed ? 'h-20' : 'h-40'} text-center p-4`}>
              {!isCollapsed ? (
                <>
                ...
                <BookOpen className="h-6 w-6 text-[#3A4D62]" />
              )}
            </div>
          ) : (
            mindboards.map((mindboard) => (
              ...
              >
                <Book className={`h-4 w-4 ${isCollapsed ? 'mx-auto' : 'mr-2'} ${
