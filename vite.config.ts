...
export default defineConfig(({ mode }) => {
  // Create the input configuration with the main entry always present
  const inputConfig: Record<string, string> = {
    main: path.resolve(__dirname, 'index.html')
  };
  
  // Only add sidepanel entry in production mode
  if (mode === 'production') {
    inputConfig.sidepanel = path.resolve(__dirname, 'public/extension/sidepanel.html');
  }
